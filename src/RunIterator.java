...
public class RunIterator implements Iterator {
	...
	// of
	// the encoding.
	RunIterator(DList runPixel) {
		// Your solution here. You may add parameters to the method signature.
		// this.current = runPixel.head;
		this.next = runPixel.head;
	}
	...
	public int[] next() {
		...
		if (!hasNext()) {
			...
			int pixelSequence[] = new int[4];
			pixelSequence[0] = next.item[0];
			pixelSequence[1] = next.item[1];
			pixelSequence[2] = next.item[2];
			pixelSequence[3] = next.item[3];
			next = next.next;
